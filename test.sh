...
usage() {
    echo "Usage: $0 [language ...] [--publish-artifacts]"
    echo "  Test one or more languages against tests/ fixtures."
    echo "  With no arguments, tests every language in languages.txt, then"
    echo "  smoke-builds registry publish artifacts (no upload)."
    echo "  --publish-artifacts  Only (or also) run publish-artifact smoke builds."
}
...
if [ -z "$SELECTED" ] && [ "$RUN_ARTIFACTS" -eq 1 ] && [ "$#" -gt 0 ]; then
    # Only --publish-artifacts (and maybe -h already handled)
    RUN_LANGUAGES=0
fi

if [ -z "$SELECTED" ]; then
    if [ "$RUN_LANGUAGES" -eq 1 ]; then
        SELECTED="$(grep -v '^[[:space:]]*#' "$LANGUAGES_FILE" | grep -v '^[[:space:]]*$')"
        # Full suite: also smoke publish artifacts unless already requested.
        RUN_ARTIFACTS=1
    fi
fi
...
if [ "$RUN_ARTIFACTS" -eq 1 ]; then
    echo
    chmod +x "$PROJECT_DIR/scripts/smoke-publish-artifacts.sh"
    "$PROJECT_DIR/scripts/smoke-publish-artifacts.sh"
fi
